    ...
    <div class="container-fluid">
        <div class="grid row sales-main-container p-lr">
            <div class="sales-container">
                ...
                <div class="quick-act-header">
                    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center">
                        <div class="mb-2 mb-sm-0">
                            <h4 class='quick-act-title'>{{ __('Edit E-Way Bill') }}</h4>
                        </div>
                ...
                <form action="{{ route('business.eway-bills.update', $eway_bill->id) }}" method="post" enctype="multipart/form-data"
                    class="ajaxform">
                    @csrf
                    @method('PUT')
                    <div class="mt-4 mb-3">
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <input type="text" name="invoiceNumber" value="{{ $eway_bill->invoice_number }}" class="form-control" readonly>
                            </div>
                            <div class="col-12 col-md-4">
                                <label class="form-label">{{ __('Invoice Date') }}</label>
                                <input type="date" name="invoiceDate" class="form-control" value="{{ $eway_bill->invoice_date->format('Y-m-d') }}">
                            </div>
                            <div class="col-12 col-md-4">
                                ...
                                <div class="d-flex align-items-center">
                                    <select required name="party_id" id="party_id" class="form-select customer-select choices-select">
                                        ...
                                        @foreach ($customers as $customer)
                                            <option value="{{ $customer->id }}" @selected($eway_bill->party_id == $customer->id)
                                ...
                                <div class="row g-3">
                                    <div class="col-12 col-md-4">
                                        ...
                                        <select name="supply_type" class="form-select">
                                            <option value="Outward" @selected($eway_bill->supply_type == 'Outward')>{{ __('Outward') }}</option>
                                            <option value="Inward" @selected($eway_bill->supply_type == 'Inward')>{{ __('Inward') }}</option>
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Sub Type') }}</label>
                                        <select name="sub_type" class="form-select">
                                            <option value="Supply" @selected($eway_bill->sub_type == 'Supply')>{{ __('Supply') }}</option>
                                            <option value="Export" @selected($eway_bill->sub_type == 'Export')>{{ __('Export') }}</option>
                                            <option value="Job Work" @selected($eway_bill->sub_type == 'Job Work')>{{ __('Job Work') }}</option>
                                            <option value="Own Use" @selected($eway_bill->sub_type == 'Own Use')>{{ __('Own Use') }}</option>
                                            <option value="Exhibition" @selected($eway_bill->sub_type == 'Exhibition')>{{ __('Exhibition / Fairs') }}</option>
                                            <option value="Others" @selected($eway_bill->sub_type == 'Others')>{{ __('Others') }}</option>
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Doc Type') }}</label>
                                        <select name="document_type" class="form-select">
                                            <option value="Tax Invoice" @selected($eway_bill->document_type == 'Tax Invoice')>{{ __('Tax Invoice') }}</option>
                                            <option value="Bill of Supply" @selected($eway_bill->document_type == 'Bill of Supply')>{{ __('Bill of Supply') }}</option>
                                            <option value="Delivery Challan" @selected($eway_bill->document_type == 'Delivery Challan')>{{ __('Delivery Challan') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card" style="min-height:auto !important">
                                    ...
                                    <div class="card-body p-3">
                                        <div class="mb-2">
                                            ...
                                            <input type="text" name="from_name" value="{{ $eway_bill->from_name }}" class="form-control" required>
                                        </div>
                                        <div class="mb-2">
                                            <label class="form-label">{{ __('GSTIN') }}</label>
                                            <input type="text" name="from_gstin" value="{{ $eway_bill->from_gstin }}" class="form-control">
                                        </div>
                                        <div class="mb-2">
                                            <label class="form-label">{{ __('Address') }}</label>
                                            <input type="text" name="from_address" value="{{ $eway_bill->from_address }}" class="form-control" required>
                                        </div>
                                        <div class="row g-2">
                                            <div class="col-6">
                                                <label class="form-label">{{ __('PIN Code') }}</label>
                                                <input type="text" name="from_pincode" value="{{ $eway_bill->from_pincode }}" class="form-control" required>
                                            </div>
                                            <div class="col-6">
                                                ...
                                                <select name="from_state" class="form-select" required>
                                                    ...
                                                    @foreach($states as $state)
                                                        <option value="{{ $state->name }}" @selected($eway_bill->from_state == $state->name)>{{ $state->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                ...
                                <div class="card" style="min-height:auto !important">
                                    ...
                                    <div class="card-body p-3">
                                        <div class="mb-2">
                                            ...
                                            <input type="text" name="to_name" id="to_name" value="{{ $eway_bill->to_name }}" class="form-control" required>
                                        </div>
                                        <div class="mb-2">
                                            <label class="form-label">{{ __('GSTIN') }}</label>
                                            <input type="text" name="to_gstin" id="to_gstin" value="{{ $eway_bill->to_gstin }}" class="form-control">
                                        </div>
                                        <div class="mb-2">
                                            <label class="form-label">{{ __('Address') }}</label>
                                            <input type="text" name="to_address" id="to_address" value="{{ $eway_bill->to_address }}" class="form-control" required>
                                        </div>
                                        <div class="row g-2">
                                            <div class="col-6">
                                                <label class="form-label">{{ __('PIN Code') }}</label>
                                                <input type="text" name="to_pincode" id="to_pincode" value="{{ $eway_bill->to_pincode }}" class="form-control" required>
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label">{{ __('State') }}</label>
                                                <select name="to_state" id="to_state" class="form-select" required>
                                                    <option value="">{{ __('Select State') }}</option>
                                                    @foreach($states as $state)
                                                        <option value="{{ $state->name }}" @selected($eway_bill->to_state == $state->name)>{{ $state->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                            ...
                            <div class="card-body p-3">
                                <div class="row g-3">
                                    <div class="col-12 col-md-4">
                                        ...
                                        <input type="text" name="transporter_name" value="{{ $eway_bill->transporter_name }}" class="form-control">
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Transporter ID') }}</label>
                                        <input type="text" name="transporter_id" value="{{ $eway_bill->transporter_id }}" class="form-control">
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Transport Mode') }}</label>
                                        <select name="transport_mode" class="form-select">
                                            <option value="Road" @selected($eway_bill->transport_mode == 'Road')>{{ __('Road') }}</option>
                                            <option value="Rail" @selected($eway_bill->transport_mode == 'Rail')>{{ __('Rail') }}</option>
                                            <option value="Air" @selected($eway_bill->transport_mode == 'Air')>{{ __('Air') }}</option>
                                            <option value="Ship" @selected($eway_bill->transport_mode == 'Ship')>{{ __('Ship') }}</option>
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Distance (KM)') }}</label>
                                        <input type="number" name="distance" min="0" value="{{ $eway_bill->distance }}" class="form-control" required>
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Vehicle Number') }}</label>
                                        <input type="text" name="vehicle_number" value="{{ $eway_bill->vehicle_number }}" class="form-control" placeholder="e.g. DL1CA1234">
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <label class="form-label">{{ __('Vehicle Type') }}</label>
                                        <select name="vehicle_type" class="form-select">
                                            <option value="Regular" @selected($eway_bill->vehicle_type == 'Regular')>{{ __('Regular') }}</option>
                                            <option value="Over Dimensional Cargo" @selected($eway_bill->vehicle_type == 'Over Dimensional Cargo')>{{ __('Over Dimensional Cargo (ODC)') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="row amount-container align-items-center mb-2">
                                        ...
                                        <div class="col-6 w-100 d-flex justify-content-between gap-2">
                                            <div class="d-flex align-items-center gap-2">
                                                <select name="vat_id" class="form-select vat_select" id='form-ware'>
                                                    ...
                                                    @foreach ($vats as $vat)
                                                        <option value="{{ $vat->id }}"
                                                            data-rate="{{ $vat->rate }}"
                                                            @selected($eway_bill->vat_id == $vat->id)>
                                                            {{ $vat->name }} ({{ $vat->rate }}%)
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <input type="number" step="any" name="tax_amount" id="tax_amount"
                                                value="{{ $eway_bill->tax_amount }}" min="0" class="form-control right-start-input"
                                                placeholder="0" readonly>
                                        </div>
                                    </div>
                                    <div class="row amount-container align-items-center mb-2">
                                        <h6 class="payment-title">{{ __('Note / Remarks') }}</h6>
                                        <input type="text" name="note" class="form-control" value="{{ $eway_bill->notes }}"
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="payment-container mb-3 amount-info-container">
                                    <div class="mb-2 d-flex align-items-center justify-content-between">
                                        <h6>{{ __('Sub Total') }}</h6>
                                        <h6 class="fw-bold" id="sub_total">{{ currency_format($eway_bill->sub_total, false) }}</h6>
                                    </div>
                                    <div class="row save-amount-container align-items-center mb-2">
                                        ...
                                        <div class="col-6 w-100">
                                            <input type="number" step="any" name="discountAmount"
                                                id="discount_amount" min="0" value="{{ $eway_bill->discount_amount }}"
                                    ...
                                    <div class="mb-2 shopping-crg-grid">
                                        ...
                                        <div class="">
                                            <input type="number" step="any" name="shipping_charge"
                                                ...
                                                value="{{ $eway_bill->shipping_charge }}" placeholder="0">
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center justify-content-between fw-bold mt-3">
                                        <div class="fw-bold">{{ __('Total Amount') }}</div>
                                        <h6 class='fw-bold' id="total_amount">{{ currency_format($eway_bill->total_amount, false) }}</h6>
                                    </div>
                                </div>
                                <div class="sale-btn-container">
                                    <button class="submit-btn payment-btn" type="submit">{{ __('Update E-Way Bill') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
